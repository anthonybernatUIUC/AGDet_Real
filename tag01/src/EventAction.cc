...
EventAction::EventAction() : G4UserEventAction(), fDecayChain(), fEvisTot(0.) {
...
void EventAction::EndOfEventAction(const G4Event* evt) {
  ...
  if (fEdepGe > 0.) { // Primary Particles (Gammas)
    man->FillNtupleDColumn(1, 0, fEdepGe / CLHEP::keV);
    man->AddNtupleRow(1);
  } else if (fEdepSi > 0) { // Secondary Particles (Alphas)
    man->FillNtupleDColumn(2, 0, fEdepSi / CLHEP::MeV);
    man->AddNtupleRow(2);
  }
}
